...
import { shareToSocialMedia, copyToClipboard } from '@/lib/social'
  ...
  const handleShare = async (platform: string) => {
    try {
      if (platform === 'copy') {
        ...
        await shareToSocialMedia(url, title, platform)
      }
      setShowMenu(false)
    } catch (error) {
          ...
          <div className="py-1">
            {socialPlatforms.map((platform) => (
              <button
                ...
                className={`flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 ${platform.color}`}
              >
